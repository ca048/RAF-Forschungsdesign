...
ego_lhl<- make_ego_graph(raf, order=1, nodes = V(raf)$name == "Entfuehrung Landshut 13.10.1977", mode = "all")
...
#Die Dichte des Netzwerks der ersten Generation beträgt also 0.3, das bedeutet, dass 30% von allen möglichen Beziehungen zwischen den Knoten realisiert wurden. Dies bedeutet auch, dass zwischen 70% der Knoten keine Beziehungen bestehen. Im Vergleich zur Dichte des Gesamtnetzwerks ist die Dichte der ersten Generation damit erheblich größer.
```
...
#Die Dichte des Netzwerks der zweiten Generation beträgt also 0.2987988, das bedeutet, dass ca. 29,88% (gerundet fast 30%) von allen möglichen Beziehungen zwischen den Knoten realisiert wurde. Dies bedeutet auch, dass zwischen ca. 71% der Knoten keine Beziehungen bestehen. Im Vergleich zur Dichte des Gesamtnetzwerks ist die Dichte der zweiten Generation damit erheblich größer. Im Vergleich zur Dichte des Netzwerks der ersten Generation ist sie fast identisch.
...
#Die Dichte beträgt 1970 circa 23,8%
...
#Die Dichte beträgt 1971 circa 15,7%, hat also zum Vorjahr um etwa 8 Prozentpunkte abgenommen.
diameter(raf_1971, weights = NA) #Pfaddistanz
#Auch die weiteste Pfaddistanz erhöht sich auf 6 und besteht zwischen Ronald Augustin und Rolf Pohle
farthest_vertices(raf_1971) #am weitesten entfernte Knoten
```

```{r Berechnungen 1972}
edge_density(raf_1972)#Dichte
#1972 beträgt die Distanz noch einmal etwa zwei Prozentpunkte weniger und liegt nun bei 13,97%, wo sie sich die folgenden Jahre einpendeln soll
...
#Die Dichte liegt weiterhin bei circa 13,6%; allein im Jahr 1975 fällt der Wert einmal auf unter 13%, aber nur knapp darunter auf 12,86% und erhöht sich im Folgejahr wieder
diameter(raf_1973, weights = NA) #Pfaddistanz
#Auch die Pfaddistanz bleibt bei 6 zwischen Ingrid Siepmann und Hans-Juergen Baecker
#Dieser Wert und die Personen ändern sich in den Folgejahren nicht mehr. Lediglich 1975 liegt die Pfaddistanz zwischen Hans-Joachim Klein und Monika Haas bei 4.
farthest_vertices(raf_1973) #am weitesten entfernte Knoten
```

```{r Berechnungen 1974}
edge_density(raf_1974)#Dichte
#Wieder bleibt die Dichte bei ungefähr 13,3%
diameter(raf_1974, weights = NA) #Pfaddistanz
farthest_vertices(raf_1974) #am weitesten entfernte Knoten
```

```{r Berechnungen 1975}
edge_density(raf_1975)#Dichte liegt knapp unter 13%, bei 12,86%
diameter(raf_1975, weights = NA) #Pfaddistanz: 1975 liegt die Pfaddistanz zwischen Hans-Joachim Klein und Monika Haas bei 4.
farthest_vertices(raf_1975) #am weitesten entfernte Knoten
```

```{r Berechnungen 1976}
edge_density(raf_1976)#Dichte; liegt bei 13,2%
diameter(raf_1976, weights = NA) #Pfaddistanz
farthest_vertices(raf_1976) #am weitesten entfernte Knoten
```

```{r Berechnungen 1977}
edge_density(raf_1977)#Dichte; liegt bei 13,1%
...
edge_density(frauen_eins)#Dichte bei circa 14,3%
diameter(frauen_eins)#Pfaddistanz bei 5
farthest_vertices(raf_1977) #am weitesten entfernte weibliche Knoten: Brigitte Asdonk & Verena Becker 
```
...
edge_density(maenner_eins)#Dichte bei circa 12,3% -> überraschend, sieht dichter aus
diameter(maenner_eins)#Pfaddistanz bei 5
```
...
edge_density(maenner_zwei)#Dichte bei circa 18,8%
