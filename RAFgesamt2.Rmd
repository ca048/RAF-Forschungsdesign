...
V(raf)$vertex.label.family <- "sans" # legt fest, dass nur Schriftarten ohne Serifen verwendet werden
V(raf)$vertex.label.cex <- .07
...
V(provision)[V(provision)$generation == 4]$color <- "lightpunk" # Unterstützer*innen werden hellrosa hervorgehoben, um zu sehen, wie viele Externe bzw. Nicht-Mitglieder eine Rolle gespielt haben
...
```{r Egonetzwerk Befreiung von Andreas Bader 1970, fig.height=10, fig.width=16, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r Ego-Netzwerk Peter-Jürgen Boock, fig.height=10, fig.width=16, message=FALSE, warning=FALSE, paged.print=FALSE}
...
ego_pb1 <- ego_pb[[1]] # notwendiger Zwischenschritt, damit das igraph-Objekt hergestellt wird
...
ego_bm
#man braucht diesen Zwischenschritt, damit das igraph-Objekt hergestellt wird
ego_bm1 <- ego_bm[[1]]
ego_bm1
...
# Die zwei Knoten, die am weitesten entfernt sind (7 Schritte) sind Christine Dümlein und Carmen Roll. Dies ist nicht sonderlich verwunderlich, da beide in verschiedenen Generationen aktiv waren (Dümlein in der zweiten, Roll in der ersten Generation). Dies zeigt, dass die RAF zwar sprachlich oft als Gesamtkonstrukt bezeichnet wird ("DIE" Rote-Armee-Fraktion), faktisch aber gerade zwischen den ersten beiden Generationen (die dritte wurde nicht betrachtet) nicht die stärksten Bindungen bestandne.
